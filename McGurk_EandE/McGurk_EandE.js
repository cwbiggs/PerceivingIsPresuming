...
var words = ["Ember and Elodie bought a terrible monster, Ba-bar, who blew himself up in order to escape them.", 
  "Ember and Elodie fought a terrible monster, Fa-far, who flew himself up in order to escape them."];
...
function setup() {
  ...
  components = new Components("McGurk Example N", borders[0], borders[1], borders[2], borders[3]);
  //create help region
  ...
  reset = new Button("Restart", 350 + borders[0], 5 + borders[1], 100, 40);
}
...
function draw() {
  ...
  if (isRunning) {
    ...
    reset.make(20, 25);
    reset.help("if audio and video becomes misaligned, click this");
  } else {
    //draw select person
  ...
  if (reset.trigger()) {
     sfMatched[person].stop();
    sfMisMatched[person].stop();
    v[person].stop();
        sfMatched[person].play();
    sfMisMatched[person].play();
    v[person].play();
  }
